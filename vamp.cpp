...
static uint8_t dst_mac[MAC_ADDR_LEN] = VAMP_BROADCAST_ADDR;
...
bool vamp_send_data(const uint8_t * data, uint8_t len) {
    ...
    /*  Enviar el mensaje */    
    if (!mac_send(payload_buffer, payload_len)) {
...
bool vamp_join_network(void) {
    ...
    uint8_t join_req_payload[MAC_ADDR_LEN + 1]; // 1 byte para el tipo de mensaje + MAC_ADDR_LEN bytes
    uint8_t payload_len = 0;
    ...
    if (!mac_send(join_req_payload, payload_len)) {
    ...
    if (join_req_payload[0] != VAMP_JOIN_ACK || payload_len < (1 + MAC_ADDR_LEN)) { 
